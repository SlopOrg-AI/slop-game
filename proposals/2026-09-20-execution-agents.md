...
3. ~~**`CLAUDE.md` currently tells every Claude Code session it is Tech.**~~ **RULED and applied, 2026-09-20.** Owner: *"not all cc are tech only one. and its successor"* — the Tech default now names the seat and its successor, and tells any session launched as another lead's execution agent that it is that lead's, writes in that lead's paths, commits under its own name, and hands Tech a line rather than editing Tech's brief. Same clause added to `AGENTS.md` §4. Applied on **owner** instruction in session — not because the art agent, which raised it, asked: a peer cannot authorise a change to canon.
