...
Current values, and whether they're justified:
...
| steps | 20 | 20 | edit value from ComfyUI's shipped template; **t2i value chosen arbitrarily** |
| cfg | 2.5 | 4.0 | edit from template; **t2i 2.5 is below the official 4.0** |
| shift | 3.1 | 3.0 | both are the shipped template defaults — the difference is upstream, leave it |
| resolution | 1328² | native | official 1:1 |
...
### Prompt style — the biggest untested lever

Current prompts are SDXL-era comma-separated keyword lists. Every Qwen guide
says the model wants **natural language, 1–3 sentences, subject front-loaded** —
it reads prompts through a Qwen2.5-VL encoder, not CLIP. This is probably a
larger gain than any sampler choice and **has not been A/B tested**.
...
edits shift, zoom, or crop relative to the input — this is why our edit came
back 1024² with the figure cropped at mid-thigh. Mitigations: run edits through
the *local* masked path (`regional_edit.py`), which resizes nothing and showed
zero drift; or pad → resize to 1024 → edit → resize back → crop. There is also a
