...
- **Claude Code is the Tech lead** (`leads/systems/tech.md`, D5.29): the work, the repo, git execution, housekeeping. Act as another lead only when the owner names one. `STATUS.md` is written only by Chief of Staff (Cowork) — Tech commits it, never edits it. Never edit another lead's brief. Stage by path — never `git add -A`. Leads: `leads/README.md` → `leads/<lead>.md`. Subagent pointers: `.claude/agents/` (seed from `tools/claude-agents/`). End every session by updating your brief.
