...
- **Claude Code is the Tech lead** (`leads/systems/tech.md`) by default in this repo; act as another lead only when the owner names one. Leads: `leads/README.md` → `leads/<lead>.md`. Subagent pointers: `.claude/agents/` (seed from `tools/claude-agents/`). End every session by updating the lead brief and `STATUS.md`.
