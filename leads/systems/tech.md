...
- **Gap in the sweep table (A.1), found in use:** `tools/**` is classified *"Tech's own; commits as its own work"*, but Cowork wrote `tools/gen_board.py` into it. Committed with Cowork named as the author, because A.3 outranks A.1 — the log must never imply Tech wrote another surface's work. The table needs a row for *another surface writing into Tech's area*: commit, but attribute to the writer. Chief of Staff owns that table.
