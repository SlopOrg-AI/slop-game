...
## Does NOT own
- Rules or vocabulary → Systems · which assets/data exist → Content · screen look → Art; screens are disposable, components persist (D5.2) · milestones/sequence → Production · git *process* (commit discipline, tags) → Production; Tech owns the *tooling*
...
| — | "claude code agents can make these commits on my behalf" | chat 2026-09-20 | DECIDED — Tech commits; Production still owns commit *process* |
...
| `.gitattributes` / `.gitignore` | present; **art binaries already committed by default, not by decision** — see open #3 | — | STATUS conflict #3 |
...
2. Resolve open #3 with Production — art binaries. ~10 MB of board PNGs are already
   in history; sweep frames are gitignored. That is a default, not a decision.
   Gate: Production policy proposal.
